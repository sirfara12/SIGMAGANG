...
<form action="{{ route('jenismagang.store') }}" method="POST" class="space-y-6 bg-white p-6 border border-gray-200 rounded-lg">
    @csrf

    <div>
        <label for="jenis_magang" class="block text-sm font-medium text-gray-900">Jenis Magang</label>
        <input type="text" id="jenis_magang" name="jenis_magang" required
    ...
    <div class="mt-6 flex items-center justify-start gap-x-6">
        ...
        <button type="button" class="text-sm font-semibold text-gray-900 hover:text-gray-900 hover:border border-gray-900 rounded-md px-3 py-2" onclick="location.href='{{ route('jenismagang.index') }}'">Batal</button>
    </div>
</form>
@endsection
